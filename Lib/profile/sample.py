    ...
    def print_stats(self, sort=-1, limit=None, show_summary=True):
        ...
        if show_summary and stats_list:
            ...
            def format_time_consuming(stat):
                ...
                if tt > 0:
                    return f"{tt * tt_scale:8.3f} {tt_unit} total time, {(tt/nc) * tt_scale:8.3f} {tt_unit} per call: {_format_func_name(func)}"
                return None
            _print_top_functions(stats_list, "Most Time-Consuming Functions", 
                               key_func=lambda x: x[3], format_line=format_time_consuming)

            # Most called functions
            def format_most_called(stat):
                func, _, nc, tt, _, _ = stat
                if nc > 0:
                    return f"{nc:8d} calls, {(tt/nc) * tt_scale:8.3f} {tt_unit} per call: {_format_func_name(func)}"
                return None
            _print_top_functions(stats_list, "Most Called Functions",
                               key_func=lambda x: x[2], format_line=format_most_called)

            # Functions with highest per-call overhead
            def format_overhead(stat):
                func, _, nc, tt, _, _ = stat
                if nc > 0 and tt > 0:
                    return f"{(tt/nc) * tt_scale:8.3f} {tt_unit} per call, {nc:8d} calls: {_format_func_name(func)}"
                return None
            _print_top_functions(stats_list, "Functions with Highest Per-Call Overhead",
                               key_func=lambda x: x[3]/x[2] if x[2] > 0 else 0, format_line=format_overhead)

            # Functions with highest cumulative impact
            def format_cumulative(stat):
                func, _, nc, _, ct, _ = stat
                if ct > 0:
                    return f"{ct * ct_scale:8.3f} {ct_unit} cumulative time, {(ct/nc) * ct_scale:8.3f} {ct_unit} per call: {_format_func_name(func)}"
                return None
            _print_top_functions(stats_list, "Functions with Highest Cumulative Impact",
                               key_func=lambda x: x[4], format_line=format_cumulative)
