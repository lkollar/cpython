...
class TestSampleProfilerComponents(unittest.TestCase):
    ...
    def test_collapsed_stack_collector_basic(self):
        """Test basic CollapsedStackCollector functionality."""
    ...
    def test_collapsed_stack_collector_export(self):
        """Test CollapsedStackCollector export functionality."""
        collector = CollapsedStackCollector()
    ...
    def test_pstats_collector_export(self):
        """Test PstatsCollector export functionality."""
        collector = PstatsCollector(
            sample_interval_usec=1000000
        )  # 1 second intervals
        ...
        with tempfile.NamedTemporaryFile(suffix=".pstats") as f:
            filename = f.name
            collector.export(filename)
...
'''
    ...
    def test_sampling_basic_functionality(self):
        """Test basic sampling functionality with a real subprocess."""
        with (
    ...
    def test_sampling_with_pstats_export(self):
        with (
            test_subprocess(self.test_script) as proc,
            tempfile.NamedTemporaryFile(suffix=".pstats") as f,
        ):
            stats_file = f.name
            ...
            self.assertGreater(os.path.getsize(stats_file), 0)
    ...
    def test_sampling_with_collapsed_export(self):
        with (
            ...
            # Verify file was created and contains valid data
            self.assertTrue(os.path.exists(collapsed_file))
            self.assertGreater(os.path.getsize(collapsed_file), 0)
    ...
    def test_sampling_all_threads(self):
        """Test sampling with all_threads option."""
        with (
            test_subprocess(self.test_script) as proc,
